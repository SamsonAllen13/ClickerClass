...
{
	internal class FMedalGauge : InterfaceResource
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			else if (clickerPlayer.accFMedalAmount > 20 && clickerPlayer.clickerSelected)
			{
				FadeTime = MAX_FADE_TIME + FADE_DELAY;
				if (clickerPlayer.accFMedalAmount > 20)
				{
					_delay++;
				}
			}
		...
		protected override bool DrawSelf()
		{
			...
			borderAsset = ClickerClass.mod.Assets.Request<Texture2D>("UI/FMedalGauge_Sheet");
			...
			float fill = (float)(clickerPlayer.accFMedalAmount) / 200;
			...
			if (frame.Contains(Main.mouseX, Main.mouseY))
			{
				...
				string text = "Charges: " + (int)(clickerPlayer.accFMedalAmount / 20) + " / 10";
				Main.instance.MouseTextHackZoom(text, Terraria.ID.ItemRarityID.LightPurple);
				Main.mouseText = true;
			}
			return true;
		}
