...
namespace ClickerClass.UI
{
	internal class PaperclipsGauge : InterfaceResource
	{
		...
		{
			MouseoverText = Language.GetOrRegister(ClickerClass.mod.GetLocalizationKey("UI.PaperclipsGauge"));
		}
		...
		protected override bool DrawSelf()
		{
			...
			float fill = (float)clickerPlayer.accPaperclipsAmount / BottomlessBoxofPaperclips.ChargeMax;
			...
			if (frame.Contains(Main.mouseX, Main.mouseY))
			{
				...
				string text = MouseoverText.Format(clickerPlayer.accPaperclipsAmount, BottomlessBoxofPaperclips.ChargeMax);
